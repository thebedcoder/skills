...
- Workflow (tier ≥ 1): show detected phase→owner mapping from `references/workflow-spec.md`. Ask only about phases scan could not resolve — unknown tooling found ("which phases does `<X>` own?"), or a phase with no owner and no obvious steps. Never ask about a phase already routed.
- Dependency constraints (tier ≥ 1): ask ONLY when scan hit ambiguity — pinned dep with no comment explaining the pin, monorepo with no declared build order, native deps present. One question, concrete: "`<pkg>` pinned at `<ver>` — deliberate? what breaks if it moves?" No ambiguity → do not ask.
