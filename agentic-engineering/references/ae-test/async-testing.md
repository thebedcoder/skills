...
```python
# Bug — async test not marked
async def test_something():   # no @pytest.mark.asyncio, asyncio_mode not "auto"
    result = await async_op()
    assert result == expected
```
This does **not** silently pass. Bare pytest emits
`PytestUnhandledCoroutineWarning` and **skips** the test; from pytest 8.4 it
fails outright. Either way the assertions never run — so it is still a real
finding, but report it as "skipped / errored, never executed", not as
"passes without asserting". Projects on `asyncio_mode = auto` need no marker at
all; check the config before flagging.
...
```javascript
// NOT a bug — a returned promise IS awaited, and a rejection fails the test.
test('fetches data', () => {
...
// Bug — the promise is NOT returned. Nothing awaits it, the test ends
// immediately, and both the assertion and any rejection are lost.
test('fetches data', () => {
    fetchData().then(data => {
        expect(data).toBeDefined()
    })
