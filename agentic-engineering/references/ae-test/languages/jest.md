...
**`jest.*` below is shorthand for whichever runner the project uses.** Vitest's
equivalents are `vi.mock`, `vi.fn`, `vi.spyOn`, `vi.useFakeTimers`,
`vi.clearAllMocks`, imported from `vitest`. **Correct `vi.*` code in a Vitest
project is not a finding** — check the project's runner before flagging an API
name, and never suggest rewriting `vi.*` to `jest.*`.
