...
**Flag:** a timeout long enough to hide a real network call (> 10s), or an `XCTestExpectation` created and never waited on — that one passes instantly and proves nothing. `waitForExpectations` always takes a `timeout:`; there is no no-timeout form to flag.
