...
// With timeout — no attribute for this in std, tokio or tokio-test.
// Wrap the future instead:
#[tokio::test]
...
// (An attribute form exists in third-party crates such as `ntest`
//  (`#[ntest::timeout(5000)]`) — only suggest it if the project already has one.)
```
