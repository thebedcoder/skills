...
- go test: `auth/auth_test.go:TestLoginRedirects` (re-run with `go test -run TestLoginRedirects ./auth`)
- xctest: `AuthTests/testLoginRedirects`
- junit: `AuthTest#testLoginRedirects`
- flutter test: `test/auth_test.dart` + the test's description string, e.g. `test/auth_test.dart > login redirects on success`. `flutter test` has no `::` id form; `--plain-name '<description>'` is how you re-run one
