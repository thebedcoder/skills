...
The `### Visual Artifacts` table in `PROGRESS.md` is the durable visual record of
a shipped story's UI behaviour, one row per capture.

**The rules for validating it live in `agents/ae-ux.md` (Step 5), not here.**
This file is the spacing / hierarchy / colour / typography checklist; it carried
80 lines of duplicated workflow policy, and two copies of a validation table
drift. Read the agent prompt for: what counts as an artifact, the per-cell
validation rules, when the section is legitimately omitted, and when
`CONSTITUTION.md` escalates a warning to a blocker.
