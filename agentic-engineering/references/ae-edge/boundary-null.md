...
**Scope vs `ae-red`.** `ae-red`'s `null-safety.md` covers the same triggers and
owns *the crash*. You own *the missing test*: a finding here must name a probe
that fails today and does not exist in the suite. A finding that only restates
"this can be null" is `ae-red`'s, and `/review` drops it as a duplicate.
