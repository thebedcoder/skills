...
A request body reaching `pickle.loads`, `yaml.load` without a safe loader,
`eval`, or a Java/`.NET` object deserializer is RCE. Full patterns per language:
`${CLAUDE_PLUGIN_ROOT}/references/ae-sec/deserialization.md`. Load that rather
than working from a partial copy here — this file used to carry one, and the two
drifted.
