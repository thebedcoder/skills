...
**Weak random:**
```swift
// NOT a finding: Swift's default RandomNumberGenerator (SystemRandomNumberGenerator)
// is documented as cryptographically secure on Apple platforms and Linux.
let random = Int.random(in: 0..<1000000)   // fine, including for security use
arc4random_uniform(1000000)                 // also cryptographically secure

// DO flag a hand-rolled or seeded generator standing in for randomness:
var seeded = SeededGenerator(seed: 42)
let token = Int.random(in: 0..<1000000, using: &seeded)   // FLAG — reproducible
...
config.defaultWebpagePreferences.allowsContentJavaScript = true  // iOS 14+
// (`preferences.javaScriptEnabled` is the deprecated iOS 13 spelling — flag the
//  behaviour either way, but don't recommend the deprecated property in a fix)
