...
The *thread-safety* half of this — class-level state mutated by concurrent
handlers — is in `${CLAUDE_PLUGIN_ROOT}/references/ae-red/async-concurrency.md`
under "Thread safety on shared mutable state". One finding, not two; report it
from whichever lens the diff actually shows.
...
**Not yours.** Test isolation is `ae-test`'s beat — a test that leaves
`MyModule.config['debug'] = True` behind is a test-quality finding, not a
runtime bug in the code under review. Skip it; `ae-test` reports it.
