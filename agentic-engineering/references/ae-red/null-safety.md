...
**Scope vs `ae-edge`.** You own the *crash on the current path*: this code, these
inputs, dereferenced without a guard. `ae-edge`'s `boundary-null.md` covers the
same triggers from the other side — *no test proves the guard holds*. When both
would fire on one line, report the crash and leave the missing test to `ae-edge`;
`/review` keeps only one of the two.
