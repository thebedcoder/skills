...
def repair_repo(config: Config, repo_name: str) -> dict:
    ...
    records = read_records(brain_root, repo_name)
    ...
    for ticket_id, rec in records.items():
        edges, dropped_e = validate_citations(rec.edge_cases_handled, source_path, pr_numbers_seen, case_ids_seen)
        gaps, dropped_g = validate_citations(rec.known_gaps, source_path, pr_numbers_seen, case_ids_seen)
        qa, dropped_q = validate_citations(rec.qa_edges, source_path, pr_numbers_seen, case_ids_seen)
        ...
        for f in rec.files:
            if f.change != "deleted" and not _head_exists(source_path, f.path):
                f.change = "deleted"
        ...
        for g in gaps:
            if "TODO" in g.source and ":" in g.source:
                ...
                if not _head_exists(source_path, path):
